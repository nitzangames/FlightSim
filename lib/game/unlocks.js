...
export class UnlockState {
  ...
  // NBucks fulfillment boundary: persist the unlock before the SDK commits the
  // wallet reservation. Plane identity itself makes replay idempotent.
  async fulfillPurchasedUnlock(key) {
    if (this._set.has(key)) return false;
    const saved = await updateKeyDurable(UNLOCKED_KEY, (currentRaw) => {
      const current = currentRaw ? parseSet(currentRaw) : new Set(this._set);
      current.add(key);
      return JSON.stringify([...current]);
    });
    this._set = parseSet(saved);
    return true;
  }
}
