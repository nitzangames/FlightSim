...
// Bilinear biome weights at (x, z): for the 4 surrounding cells in the
// 3×3 lookup, compute their interpolation weights, aggregate by biome
// NAME (cells with the same biome — e.g. two forest cells — sum their
// weights), and return the dominant biome plus its weight in [0, 1].
//
// weight == 1.0 means the chunk is in the interior of a single biome
// region. weight ≈ 0.5 means it straddles a boundary between two
// roughly-equal biomes. Used by scatter placement to suppress trees /
// cacti / etc. in transition zones, so you don't see forest trees in a
// chunk whose ground colours are 50% arctic.
export function biomeDominanceAt(x, z) {
  const t = noise(x + TEMP_OFFSET.x,  z + TEMP_OFFSET.z,  TEMP_SCALE)  * 3;
  const m = noise(x + MOIST_OFFSET.x, z + MOIST_OFFSET.z, MOIST_SCALE) * 3;
  const ti  = Math.min(2, Math.floor(t));
  const mi  = Math.min(2, Math.floor(m));
  ...
  const w01 = (1 - tf) * mf;
  const w11 = tf * mf;
  const cells   = [LOOKUP[ti][mi], LOOKUP[ti1][mi], LOOKUP[ti][mi1], LOOKUP[ti1][mi1]];
  const weights = [w00, w10, w01, w11];
  // Aggregate by name. With only 5 biomes and 4 cells, a small fixed map
  // is fine — Map ordering preserves insertion, so the first observed
  // name wins ties (rare, but deterministic).
  const sums = new Map();
  for (let i = 0; i < 4; i++) {
    const name = cells[i].name;
    sums.set(name, (sums.get(name) || 0) + weights[i]);
  }
  let bestName = cells[0].name, bestW = sums.get(bestName);
  for (const [name, w] of sums) {
    if (w > bestW) { bestName = name; bestW = w; }
  }
  return { biome: NAME_TO_BIOME[bestName], weight: bestW };
}
