...
export const PLANES = {
  ...
  // F-86 caps the early ramp at 10,000 ★; from the F-4 onward the price
  // steps up by 5,000 ★ per plane (see the note above the F-22).
  ...
  f4:       { key:'f4',       name:'F-4 Phantom',   build: buildF4,         unlock: stars(15000),    stats: { maxSpeed: 155, maxPitchRate: 0.50, maxYawRate: 0.50, fuelDrainRate: 0, maxFuel: 1, collisionRadius: 6.2, vertRadius: 1.8 } },
  ...
  a10:      { key:'a10',      name:'A-10 Warthog',  build: buildA10,        unlock: stars(20000),    stats: { maxSpeed: 120, maxPitchRate: 0.65, maxYawRate: 0.60, fuelDrainRate: 0, maxFuel: 1, collisionRadius: 8.5, vertRadius: 1.6 } },
  // F-16 is the 4th-gen "viper" — very agile fly-by-wire single-engine.
  f16:      { key:'f16',      name:'F-16 Falcon',   build: buildF16,        unlock: stars(25000),    stats: { maxSpeed: 145, maxPitchRate: 0.65, maxYawRate: 0.75, fuelDrainRate: 0, maxFuel: 1, collisionRadius: 4.6, vertRadius: 2.6 } },
  // F-18 is heavier twin-engine carrier fighter, less raw agility than F-16
  // but excellent high-alpha character.
  f18:      { key:'f18',      name:'F-18 Hornet',   build: buildF18,        unlock: stars(30000),    stats: { maxSpeed: 145, maxPitchRate: 0.55, maxYawRate: 0.65, fuelDrainRate: 0, maxFuel: 1, collisionRadius: 4.6, vertRadius: 2.2 } },
  f15:      { key:'f15',      name:'F-15 Eagle',    build: buildF15,        unlock: stars(35000),    stats: { maxSpeed: 175, maxPitchRate: 0.35, maxYawRate: 0.40, fuelDrainRate: 0, maxFuel: 1, collisionRadius: 5.4, vertRadius: 2.0 } },
  // From the F-4 onward each plane costs 5,000 ★ more than the last (F-4 15K,
  // A-10 20K, … SR-71 45K). The NBucks→stars store lets players top up toward
  // the higher modern-tier prices.
  f22:      { key:'f22',      name:'F-22 Raptor',   build: buildF22,        unlock: stars(40000),    stats: { maxSpeed: 165, maxPitchRate: 0.70, maxYawRate: 0.75, fuelDrainRate: 0, maxFuel: 1, collisionRadius: 4.5, vertRadius: 1.3 } },
  ...
  sr71:     { key:'sr71',     name:'SR-71 Blackbird', build: buildSR71,     unlock: stars(45000),    stats: { maxSpeed: 300, maxPitchRate: 0.25, maxYawRate: 0.30, fuelDrainRate: 0, maxFuel: 1, collisionRadius: 5.6, vertRadius: 1.2 } },
};
