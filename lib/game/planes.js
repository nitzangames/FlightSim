...
// either 'free' (always unlocked, biplane only) or 'stars' (deducted from
// the in-game ScoreTracker). NBucks (platform hard currency) no longer gate
// any plane directly — players buy stars with NBucks in the menu store and
// spend those stars here. menu.js still calls PlaySDK.nbucks.spend for that.
...
export const PLANES = {
  ...
  // Flat 10K tier: every plane past the P-51 costs 10,000 ★. The roster
  // still reads as a historical progression, but the price stops climbing
  // so any post-P-51 jet is reachable from the same star budget (and the
  // NBucks→stars store lets players top up toward it).
  ...
  f4:       { key:'f4',       name:'F-4 Phantom',   build: buildF4,         unlock: stars(10000),    stats: { maxSpeed: 155, maxPitchRate: 0.50, maxYawRate: 0.50, fuelDrainRate: 0, maxFuel: 1, collisionRadius: 6.2, vertRadius: 1.8 } },
  ...
  a10:      { key:'a10',      name:'A-10 Warthog',  build: buildA10,        unlock: stars(10000),    stats: { maxSpeed: 120, maxPitchRate: 0.65, maxYawRate: 0.60, fuelDrainRate: 0, maxFuel: 1, collisionRadius: 8.5, vertRadius: 1.6 } },
  // F-16 is the 4th-gen "viper" — very agile fly-by-wire single-engine.
  f16:      { key:'f16',      name:'F-16 Falcon',   build: buildF16,        unlock: stars(10000),    stats: { maxSpeed: 145, maxPitchRate: 0.65, maxYawRate: 0.75, fuelDrainRate: 0, maxFuel: 1, collisionRadius: 4.6, vertRadius: 2.6 } },
  // F-18 is heavier twin-engine carrier fighter, less raw agility than F-16
  // but excellent high-alpha character.
  f18:      { key:'f18',      name:'F-18 Hornet',   build: buildF18,        unlock: stars(10000),    stats: { maxSpeed: 145, maxPitchRate: 0.55, maxYawRate: 0.65, fuelDrainRate: 0, maxFuel: 1, collisionRadius: 4.6, vertRadius: 2.2 } },
  f15:      { key:'f15',      name:'F-15 Eagle',    build: buildF15,        unlock: stars(10000),    stats: { maxSpeed: 175, maxPitchRate: 0.35, maxYawRate: 0.40, fuelDrainRate: 0, maxFuel: 1, collisionRadius: 5.4, vertRadius: 2.0 } },
  // F-22 + SR-71 are no longer NBucks-gated — they cost the same flat 10K ★
  // as the rest of the modern tier. Players who want them faster can buy
  // stars with NBucks in the menu store.
  f22:      { key:'f22',      name:'F-22 Raptor',   build: buildF22,        unlock: stars(10000),    stats: { maxSpeed: 165, maxPitchRate: 0.70, maxYawRate: 0.75, fuelDrainRate: 0, maxFuel: 1, collisionRadius: 4.5, vertRadius: 1.3 } },
  ...
  sr71:     { key:'sr71',     name:'SR-71 Blackbird', build: buildSR71,     unlock: stars(15000),    stats: { maxSpeed: 300, maxPitchRate: 0.25, maxYawRate: 0.30, fuelDrainRate: 0, maxFuel: 1, collisionRadius: 5.6, vertRadius: 1.2 } },
};
