...
export class ScoreTracker {
  ...
  async fulfillPurchasedStars(amount, receiptId) {
    ...
    const previousStars = this.stars;
    this.stars += amount;
    if (id) this._fulfilledNbucksReceipts.add(id);
    try {
      await saveKeyDurable(
        STARS_KEY,
        serializedStarState(this.stars, this._fulfilledNbucksReceipts),
      );
      return true;
    } catch (error) {
      this.stars = previousStars;
      if (id) this._fulfilledNbucksReceipts.delete(id);
      throw error;
    }
  }
