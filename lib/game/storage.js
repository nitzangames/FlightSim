...
// Durable read/modify/write for receipt-bearing purchase fulfillment. On the
// platform updateSave retries against the newest cloud value after conflicts;
// local development keeps the same API with a simple load + save fallback.
export async function updateKeyDurable(key, updater) {
  if (typeof window !== 'undefined' && window.PlaySDK &&
      typeof window.PlaySDK.updateSave === 'function') {
    return window.PlaySDK.updateSave(key, updater);
  }
  const current = await loadKey(key);
  const next = String(await updater(current));
  await saveKeyDurable(key, next);
  return next;
}

export function saveKey(key, value) {
  // Most score/settings writes are intentionally best-effort and fire-and-forget.
  // Purchase fulfillment uses updateKeyDurable so a failed persistence write
  // rejects the fulfillment and releases the wallet reservation.
