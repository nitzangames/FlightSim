...
export function buildMenu({
  ...
  async function attemptUnlock() {
    ...
    if (p.unlock.kind === 'stars') {
      ...
      const requestedPlane = active;
      busy = true;
      try {
        await window.PlaySDK.nbucks.spend({
          amount: p.unlock.amount,
          itemDescription: `Unlock ${p.name}`,
          itemId: `plane.${requestedPlane}`,
          fulfill: async () => {
            await unlockState.fulfillPurchasedUnlock(requestedPlane);
          },
        });
        paint();
        if (active === requestedPlane) onPlaneChange && onPlaneChange(requestedPlane);
      } catch {
    ...
    try {
      await window.PlaySDK.nbucks.spend({
        ...
        fulfill: async (result) => {
          await scoreTracker.fulfillPurchasedStars(pkg.stars, result?.receiptId);
        },
      });
