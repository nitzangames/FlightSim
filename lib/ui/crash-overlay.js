...
export function buildCrashOverlay({ root, durationMs = 1500, onComplete, subtext = null }) {
  ...
  title.style.cssText = 'font-size:11cqi;font-weight:900;letter-spacing:0.1em;';
  title.textContent = 'CRASHED';
  wrap.appendChild(title);
  // Optional debug line under the crash text (e.g. impact angle on localhost).
  if (subtext) {
    const sub = document.createElement('div');
    sub.style.cssText = 'margin-top:2cqi;font-size:3.4cqi;font-weight:700;letter-spacing:0.04em;opacity:0.85;font-family:ui-monospace,Menlo,monospace;';
