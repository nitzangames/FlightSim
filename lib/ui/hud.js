...
//   • Back arrow (bottom-left, aligned with readouts row) — returns to MENU.
...
export function buildHUD({ root, version, onBack }) {
  ...
  wrap.innerHTML = `
    <button data-back style="position:absolute;bottom:8%;left:4%;pointer-events:auto;background:rgba(0,0,0,0.35);color:#fff;border:1px solid rgba(255,255,255,0.25);border-radius:50%;width:9cqi;height:9cqi;font-size:5cqi;cursor:pointer;">◄</button>
  ...
  function update({ speed, altitude, countdown, villageBearing, villageDistance, villageType, villageVisited, stalling, visitedCount, visitedTotal, flashT, stars, bonuses, trickFlash, biome, planeX, planeZ, heading, visitedPois }) {
