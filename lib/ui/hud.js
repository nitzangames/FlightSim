...
export function buildHUD({ root, version, onSettings }) {
  ...
  wrap.innerHTML = `
    <button data-settings aria-label="Settings" style="position:absolute;top:3.5%;left:4%;pointer-events:auto;background:rgba(0,0,0,0.35);color:#fff;border:1px solid rgba(255,255,255,0.25);border-radius:50%;width:9cqmin;height:9cqmin;font-size:5cqmin;line-height:1;cursor:pointer;display:flex;align-items:center;justify-content:center;">⚙</button>
    <div data-village-marker style="position:absolute;top:3.5%;left:0;right:0;text-align:center;display:none;">
      <svg data-village-arrow viewBox="0 0 24 32" style="display:inline-block;width:7cqmin;height:9.3cqmin;color:#ffe;filter:drop-shadow(0 1px 3px rgba(0,0,0,0.85));transform-origin:50% 50%;">
        <polygon points="12,1 22,29 12,22 2,29" fill="currentColor"/>
      </svg>
      <div data-village-dist style="font-size:2.8cqmin;font-weight:600;letter-spacing:0.5px;color:#ffe;text-shadow:0 1px 4px rgba(0,0,0,0.8);margin-top:0.4cqmin;"></div>
      <div data-village-label style="font-size:2.0cqmin;opacity:0.7;letter-spacing:1px;">VILLAGE</div>
    </div>
    <div data-readouts style="position:absolute;bottom:8%;left:18%;font-size:3.6cqmin;line-height:1.4;text-shadow:0 1px 4px rgba(0,0,0,0.7);">
    ...
    <div data-stars style="position:absolute;top:3.5%;right:4%;text-align:right;font-size:6cqmin;font-weight:800;letter-spacing:0.05em;color:#ffd766;text-shadow:0 2px 8px rgba(0,0,0,0.85);">0 ★</div>
    <div data-bonuses style="position:absolute;top:13%;right:4%;text-align:right;font-size:2.6cqmin;font-weight:700;letter-spacing:0.06em;color:#ffd766;text-shadow:0 1px 4px rgba(0,0,0,0.8);line-height:1.5;"></div>
    <canvas data-minimap width="160" height="160" style="position:absolute;bottom:8%;right:4%;width:22cqmin;height:22cqmin;background:rgba(8,12,18,0.55);border:1px solid rgba(255,255,255,0.25);border-radius:6px;"></canvas>
    <div data-trick-flash style="position:absolute;top:16%;left:0;right:0;text-align:center;font-size:8cqmin;font-weight:900;letter-spacing:0.1em;color:#ffd766;text-shadow:0 2px 10px rgba(0,0,0,0.85);display:none;"></div>
    <div data-marker-flash style="position:absolute;top:24%;left:0;right:0;text-align:center;font-size:6.5cqmin;font-weight:900;letter-spacing:0.15em;color:#80ffa0;text-shadow:0 2px 10px rgba(0,0,0,0.8);display:none;">FOUND</div>
    <div data-countdown style="position:absolute;top:38%;left:0;right:0;text-align:center;font-size:24cqmin;font-weight:900;letter-spacing:0.05em;text-shadow:0 2px 12px rgba(0,0,0,0.8);display:none;"></div>
    <div data-stall style="position:absolute;top:18%;left:0;right:0;text-align:center;font-size:7cqmin;font-weight:900;letter-spacing:0.2em;color:#ff5c2c;text-shadow:0 2px 8px rgba(0,0,0,0.85);display:none;">STALL</div>
    <div data-pilots style="position:absolute;bottom:5%;left:0;right:0;text-align:center;font-size:2.4cqmin;letter-spacing:0.1em;color:#7aa7d6;text-shadow:0 1px 4px rgba(0,0,0,0.7);display:none;"></div>
    <div style="position:absolute;bottom:2%;left:0;right:0;text-align:center;font-size:2.1cqmin;opacity:0.5;">${version}</div>
  `;
  ...
  function update({ speed, altitude, countdown, villageBearing, villageDistance, villageType, villageVisited, stalling, visitedCount, visitedTotal, flashT, stars, bonuses, trickFlash, biome, planeX, planeZ, heading, visitedPois, pilots }) {
    ...
    // Clock-based so the rate doesn't scale with display refresh rate.
    if (stalling) {
      elStall.style.display = (performance.now() % 333) < 167 ? '' : 'none';
    } else {
