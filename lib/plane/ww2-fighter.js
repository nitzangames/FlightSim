...
export function buildWW2Fighter(THREE) {
  ...
  // Wing tops (L + R). Wing extrudes y∈[-0.30, -0.10]; sit just above the top.
  for (const sx of [-3.2, 3.2]) {
    const r = makeRoundel(THREE, wingRoundelD);
    r.position.set(sx, -0.08, -1.3);
    g.add(r);
  }
  ...
  for (const sx of [-3.2, 3.2]) {
    ...
    r.position.set(sx, -0.32, -1.3);
    g.add(r);
  }

  // Fuselage sides (L + R) — rotate so the colored face points OUTWARD.
  // For side=+1 we want face normal +X, achieved by rotation.z = -π/2.
  for (const side of [-1, 1]) {
    const r = makeRoundel(THREE, fuseRoundelD);
    r.rotation.z = -side * Math.PI / 2;
    r.position.set(side * 0.58, 0.0, 1.6);
    g.add(r);
  }
