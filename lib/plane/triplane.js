...
export function buildTriplane(THREE) {
  ...
  // Iron Cross (Balkenkreuz) insignia — top wing top L+R only (visible
  // from above) + bottom wing bottom L+R only (visible from below).
  for (const sx of [-2.0, 2.0]) {
    const cross = makeIronCross(THREE, 0.95);
    cross.position.set(sx, 1.67, -1.10);
    g.add(cross);
  }
  for (const sx of [-1.8, 1.8]) {
    const cross = makeIronCross(THREE, 0.85);
    cross.rotation.z = Math.PI;
    cross.position.set(sx, -0.49, -1.00);
    g.add(cross);
  }
