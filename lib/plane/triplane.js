...
function makeIronCross(THREE, size) {
...
export function buildTriplane(THREE) {
  ...
  const tailV = new THREE.Mesh(finGeo, mat(RED));
  ...
  // Iron Cross (Balkenkreuz) insignia — top wing top L+R, bottom wing
  // bottom L+R, fuselage sides L+R.
  for (const sx of [-2.0, 2.0]) {
    const cross = makeIronCross(THREE, 0.95);
    cross.position.set(sx, 1.59, -1.10);    // top wing top (y = 1.50 + 0.16/2 + small offset)
    g.add(cross);
  }
  for (const sx of [-1.8, 1.8]) {
    const cross = makeIronCross(THREE, 0.95);
    cross.rotation.z = Math.PI;             // face down
    cross.position.set(sx, -0.49, -1.00);   // bottom wing bottom (y = -0.48 - 0.01)
    g.add(cross);
  }
  ...
  // Vertical fin sides — even smaller cross on each face.
  for (const side of [-1, 1]) {
    const cross = makeIronCross(THREE, 0.30);
