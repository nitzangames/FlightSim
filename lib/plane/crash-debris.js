...
export class CrashDebris {
  ...
  start(planeMesh, physics) {
    ...
    // 18% of impact velocity carries forward into the debris field —
    // less than full so pieces don't streak off camera before gravity
    // catches them. (Dialed down from 30%: at high cruise speeds the
    // wreckage was flinging out too fast, especially on tall structures.)
    const baseV = new T.Vector3(
      physics.forward.x, physics.forward.y, physics.forward.z,
    ).multiplyScalar(physics.speed * 0.18);
    ...
    for (const child of children) {
      ...
      // Radial spread + upward bias so the wreckage field is visibly broad
      // rather than collapsing toward one spot. Radial kick dialed down from
      // 10–22 to 5–12 m/s so pieces tumble out rather than shooting away; the
      // upward pop is kept so the wreckage still arcs satisfyingly.
      ...
      const velocity = baseV.clone()
        .addScaledVector(radial, 5 + Math.random() * 7)
        .add(new T.Vector3(
          (Math.random() - 0.5) * 6,
          6 + Math.random() * 9,
          (Math.random() - 0.5) * 6,
