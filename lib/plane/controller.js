...
// world up — the "horizontal component of lift" pilots call a coordinated
// turn. Linear in up.x (≈ sin bank_angle) so turn rate scales monotonically
// with how much you've rolled. 1.5 rad/s at knife-edge = ~85°/sec yaw, which
// reads as a snappy game-y turn while still recognisably "physical".
const BANK_TURN_RATE = 1.5;
...
export class PlanePhysics {
  ...
  update({ dragX, dragY, dt }) {
    ...
    // Bank-to-turn: right-banked (up.x > 0) plane yaws right around WORLD up.
    // Linear in up.x = sin(bank_angle): 30°→0.5, 60°→0.87, 90°→1. The previous
    // version multiplied by up.y too, which made turn rate peak at 45° bank
    // and then DROP toward 0 at knife-edge — wrong feel. Gate only on full
    // inversion so barrel rolls don't reverse the yaw mid-roll.
    const isInverted = this.up.y < -0.1;
    const bankYaw = isInverted ? 0 : -BANK_TURN_RATE * this.up.x;
    if (bankYaw !== 0 && dt > 0) {
