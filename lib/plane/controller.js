...
// Reduced from 14 → 5 so a 30–60° coordinated turn does not bleed altitude
// fast enough to plant the plane into rising terrain mid-turn. (Real planes
// pull back to compensate; with no rudder/elevator coordination we just
// reduce the loss rate instead.)
const LIFT_LOSS_RATE = 5;
// Bank-to-turn scaling: real physics is yawRate = (g/V) * tan(β). Tuned k
// gives the MSFS feel — more roll → more turn, monotonically. K=1.8 lets
// a 30° bank produce ~60°/sec yaw, so the player can turn briskly at gentle
// banks without bleeding altitude. Capped at tan ≈ 3 (~72° bank) so 89°
// does not blow up and so auto-yaw stops as the plane approaches knife-edge.
...
export class PlanePhysics {
  ...
  update({ dragX, dragY, dt }) {
    ...
    // MSFS-style bank-to-turn — yawRate = (g/V) * tan(bank), the real-physics
    // formula for a coordinated turn. tan() is monotonic in bank: more roll →
    // more turn, smoothly, all the way up to ~knife-edge. Active only when
    // the plane is upright (up.y > 0.05) — past knife-edge the lift vector
    // points down, no physical turning force, so the gate also keeps barrel
    // rolls from producing direction-flipping yaw mid-rotation.
    //
    // Bank is measured via plane.right.y (NOT up.x) — right.y is invariant
    // under yaw whereas up.x flips sign as the plane heading crosses 90°,
    // which broke the previous version (auto-yaw would reverse after a U-turn).
    //   sin(bank), signed = -right.y   (negative right.y = right wing dipped = right bank)
    //   cos(bank)         = up.y
    //   tan(bank)         = -right.y / up.y
    if (this.up.y > 0.05) {
      let tanBank = -this.right.y / this.up.y;
