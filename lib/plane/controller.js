...
export class PlanePhysics {
  ...
  update({ dragX, dragY, dt }) {
    ...
    if (this.speed < stallSpeed && !this.engineOff) {
      ...
      // stalled wing can't hold the nose up, so it falls through the horizon
      // on its own and the plane recovers speed by diving. World-frame (not
      // body-frame) so a banked stall still drops earthward, and it eases off
      // as the nose approaches straight-down. A floor on the rate keeps even a
      // shallow stall visibly nosing over.
      const noseRate = STALL_NOSE_DOWN_RATE * Math.max(0.4, severity);
