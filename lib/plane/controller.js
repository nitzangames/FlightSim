...
export class PlanePhysics {
  ...
  update({ dragX, dragY, dt }) {
    ...
    // No auto-yaw. Turning is "real flight sim": bank with roll, then pull
    // back on pitch. Because pitch rotates around the LOCAL right-wing axis
    // — which is tilted when the plane is banked — pulling back rotates the
    // nose in a tilted plane. Projected onto the horizon, that's a coordinated
    // turn in the bank direction. This is how a real fixed-wing plane works.
    //
    // The previous bank-state-based auto-yaw broke during barrel rolls (mid-
    // roll the plane is briefly in a "left bank" state per up.x even when
    // you're rolling right, so auto-yaw flipped direction) and felt asymmetric.
