...
export function buildBiplane(THREE) {
  ...
  // Yellow fuselage stripe — split into 2 segments per side so each
  // segment can sit close to the local body radius (the body tapers
  // along Z, so a single long stripe at constant X floats off the
  // narrowing aft body).
  // Forward segment: body radius ~0.43 around z=-0.55. Place at X=±0.44.
  // Aft segment: body radius ~0.34 around z=+1.20. Place at X=±0.34.
  for (const sx of [-1, 1]) {
    const fwd = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.16, 1.90), mat(YELLOW));
    fwd.position.set(sx * 0.44, 0.05, -0.55);
    g.add(fwd);
    const aft = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.14, 1.60), mat(YELLOW));
    aft.position.set(sx * 0.34, 0.05, 1.20);
    g.add(aft);
  }
