...
export function buildBiplane(THREE) {
  ...
  // Diagonal undercarriage rods — each runs from its wheel hub up to a shared
  // apex on the centreline, so the two rods MEET there (a proper V-frame)
  // instead of leaving a gap between their tops.
  const APEX_Y = -0.60, AXLE_Y = -1.10, GEAR_Z = -1.60;
  for (const sx of [-0.85, 0.85]) {
    const dx = sx, dy = AXLE_Y - APEX_Y;          // apex(0,APEX_Y) → wheel(sx,AXLE_Y)
    const len = Math.hypot(dx, dy);
    const rod = new THREE.Mesh(new THREE.BoxGeometry(len, 0.05, 0.05), mat(STEEL));
    rod.position.set(sx / 2, (APEX_Y + AXLE_Y) / 2, GEAR_Z);
