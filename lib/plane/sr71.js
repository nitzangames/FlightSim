...
export function buildSR71(THREE) {
  ...
  const NAC_INBOARD  = 1.95;   // X = NAC_X - NAC_R (2.5 - 0.55)
  const NAC_OUTBOARD = 3.05;   // X = NAC_X + NAC_R
  ...
  outerWing.lineTo( 5.8, -3.0);              // tip LE
  outerWing.lineTo( 5.8, -3.5);              // cropped tip TE
  ...
  const NAC_X   = 2.5;        // spanwise position
  const NAC_Y   = -0.20;
  ...
  for (const side of [-1, 1]) {
    ...
    // Inlet spike cone — 50% bigger (R: 0.18→0.27, length: 1.4→2.1).
    const spike = new THREE.Mesh(new THREE.ConeGeometry(0.27, 2.1, 12), mat(BLACK));
    spike.rotation.x = -Math.PI / 2;
    spike.position.set(side * NAC_X, NAC_Y, NAC_FRONT - 1.05);
    g.add(spike);
