...
export function buildA10(THREE) {
  ...
  // Long straight wings — plain BoxGeometry for the A-10's characteristic
  // squared-off wingtips (with separate Hoerner fences at the very ends).
  // DARK_GREEN for camo contrast against the OLIVE fuselage.
  const wingGeo = new THREE.BoxGeometry(17.0, 0.22, 2.6);
  ...
  // Trapezoidal fin with a flat top (no rounding) — A-10's vertical
  // stabs have a squared-off silhouette, not rounded tips.
  const finShape = new THREE.Shape();
  finShape.moveTo(-0.55, 0.00);   // base forward
  finShape.lineTo(+0.55, 0.00);   // base aft
  finShape.lineTo(+0.40, 1.50);   // top aft
  finShape.lineTo(-0.10, 1.50);   // top forward
