...
export function buildA10(THREE) {
  ...
  // Nose / gun housing — slightly tapered cylinder that runs from forward
  // of the body (z=-6.30) all the way back into the body (z=-4.50). Read
  // as a substantial gun mount structure, not a small floating cone.
  // Radius 0.30→0.40 small enough to fit inside the body cross-section
  // at z=-4.5 (body half-height ~0.475 there).
  const noseCone = new THREE.Mesh(new THREE.CylinderGeometry(0.30, 0.40, 1.80, 10), mat(DARK_GREEN));
  noseCone.rotation.x = Math.PI / 2;
  noseCone.position.set(0, 0.0, -5.40);
  g.add(noseCone);

  // GAU-8 cannon barrel — long thin cylinder. Extends DEEP into the
  // fuselage (aft end at z=-3.20, well inside the body) so it reads as
  // a structurally-mounted weapon rather than a floating stick.
  const cannon = new THREE.Mesh(new THREE.CylinderGeometry(0.10, 0.10, 4.0, 10), mat(BARREL));
  cannon.rotation.x = Math.PI / 2;
  cannon.position.set(0, -0.15, -5.20);
  g.add(cannon);
