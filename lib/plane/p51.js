...
export function buildP51(THREE) {
  ...
  // Spinner — long pointed yellow cone, base buried inside the cowling
  // band so the cone and cowling read as one continuous yellow nose.
  // openEnded: true skips the back-cap face (would be hidden inside the
  // cylinder anyway). Tip aligned with the propeller plane at z=-6.85.
  const spinner = new THREE.Mesh(new THREE.ConeGeometry(0.42, 2.0, 10, 1, true), mat(YELLOW));
  spinner.rotation.x = -Math.PI / 2;
  spinner.position.z = -5.85;
  g.add(spinner);
