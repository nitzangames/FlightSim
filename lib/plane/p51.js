...
export function buildP51(THREE) {
  ...
  // a low trapezoid (height 0.30): TOP edge (length 1.8) longer than BOTTOM
  // edge (length 1.0). The wide upper face EMBEDS into the fuselage
  // underside (top at world Y=-0.40, body underside at z=0.6 is ~-0.55, so
  // ~0.15 of overlap) so the scoop reads as a fairing fused to the body,
  // not a part hanging off it. Narrower lower face is the radiator outlet.
  //
  ...
  // (along world X). Position offsets:
  //   position.x = +0.425 (compensates for extrude going local 0→0.85 → world X 0→-0.85)
  //   position.y = -0.70  (shape Y goes 0→0.30, want world TOP at -0.40 → bottom at -0.70)
  //   position.z =  0.6   (shape X centered at 0, want world Z 0.6)
  const scoopShape = new THREE.Shape();
  scoopShape.moveTo(-0.5, 0.00);   // bottom-forward corner (short bottom edge)
  scoopShape.lineTo( 0.5, 0.00);   // bottom edge (length 1.0, radiator outlet)
  scoopShape.lineTo( 0.9, 0.30);   // angled rear fairing → top-rear
  scoopShape.lineTo(-0.9, 0.30);   // top edge (length 1.8, embedded into fuselage)
  ...
  scoop.position.set(0.425, -0.70, 0.6);
  g.add(scoop);
