...
export function buildP51(THREE) {
  ...
  // STRAIGHT leading edge across the entire span (no inboard/outboard
  // kink), trailing edge sweeps forward from each tip to the root center,
  // rounded tips via small half-ellipse arcs.
  const SPAN = 11.0;
  ...
  wingShape.moveTo(-TIP_X + TIP_RADIUS_X, -TIP_HALF);
  // Left rounded tip — half-ellipse arc, TE → LE.
  for (let i = 1; i < N_TIP; i++) {
    const t = i / N_TIP;
    const theta = -Math.PI / 2 + t * Math.PI;
    const x = -TIP_X + TIP_RADIUS_X * Math.cos(theta);
    const y = TIP_HALF * Math.sin(theta);
    wingShape.lineTo(x, y);
  }
  wingShape.lineTo(-TIP_X + TIP_RADIUS_X, TIP_HALF);
  // STRAIGHT leading edge across the full span (no root kink).
  wingShape.lineTo(TIP_X - TIP_RADIUS_X, TIP_HALF);
  // Right rounded tip — half-ellipse arc, LE → TE.
  for (let i = 1; i < N_TIP; i++) {
    const t = i / N_TIP;
    const theta = Math.PI / 2 - t * Math.PI;
    const x = TIP_X + TIP_RADIUS_X * Math.cos(theta);
  ...
  // Single kink at the centerline (root), no inboard "shoulder".
  ...
  // Belly radiator scoop — shaped duct via ExtrudeGeometry. Side profile is
  // a trapezoid: bottom edge (length 1.8) is longer than top edge (length
  // 1.0), so the front and rear faces angle inward as they go up. Reads
  // as an aerodynamic intake/exhaust duct rather than a brick.
  //
  // Shape coords: shapeX = world Z (front-back), shapeY = world Y (up-down).
  // ExtrudeGeometry extrudes along local +Z. After mesh.rotation.y = -π/2,
  // local +Z → world -X, so the extruded depth becomes the scoop's width
  // (along world X). Scoop center at world (0, -0.78, 0.6) needs:
  //   position.x = +0.425 (compensates for extrude going local 0→0.85 → world X 0→-0.85)
  //   position.y = -1.03  (shape Y goes 0→0.50, center at 0.25, want world center -0.78)
  //   position.z =  0.6   (shape X centered at 0, want world Z 0.6)
  const scoopShape = new THREE.Shape();
  scoopShape.moveTo(-0.9, 0.0);   // bottom-forward corner (world Z=-0.9, Y=0)
  scoopShape.lineTo( 0.9, 0.0);   // bottom edge to bottom-rear (world Z=+0.9, Y=0)
  scoopShape.lineTo( 0.5, 0.5);   // angled rear face → top-rear (world Z=+0.5, Y=+0.5)
  scoopShape.lineTo(-0.5, 0.5);   // top edge (shorter than bottom) → top-forward
  // ExtrudeGeometry implicitly closes back to start via the angled forward face.
  ...
  scoop.position.set(0.425, -1.03, 0.6);
  g.add(scoop);
  ...
  // Wings: top + bottom at ±3.2 along the span. ExtrudeGeometry extrudes
  // along local +Z, which maps to world +Y after the -π/2 X rotation —
  // so with wing.position.y = -0.45 the wing occupies y ∈ [-0.45, -0.25].
  // World wing TOP surface = -0.25; BOTTOM = -0.45. Place insignia just
  // proud of those surfaces to avoid z-fight.
  ...
  for (const side of [-1, 1]) {
    ...
    ins.position.set(side * 0.52, 0.05, 1.3);
    g.add(ins);
  }
