...
export function buildP51(THREE) {
  ...
  // Low-wing mount with wing root at the forward third of the fuselage
  // (P-51D proportions — wing LE at ~37% from spinner tip, not mid-body).
  wing.position.set(0, -0.45, -1.8);
  g.add(wing);

  // Bubble canopy — sits on top of the fuselage just aft of the wing root.
  // Body spine at z=-0.9 is ~0.59, so a canopy base at y≈0.5 sits slightly
  // embedded in the body. Stretched along Z for the teardrop look.
  const canopy = new THREE.Mesh(
    new THREE.SphereGeometry(0.42, 12, 8, 0, Math.PI * 2, 0, Math.PI / 2), mat(CANOPY),
  );
  canopy.position.set(0, 0.50, -0.9);
  ...
  //   position.z = -0.9   (aft of new wing-root TE at z≈-0.2)
  ...
  scoop.position.set(0.425, -0.78, -0.9);
  g.add(scoop);
  ...
  for (const sx of [-3.2, 3.2]) {
    ...
    ins.position.set(sx, -0.23, -1.8);
    g.add(ins);
  }
  for (const sx of [-3.2, 3.2]) {
    const ins = makeStarBar(THREE, 0.8);
    ins.rotation.z = Math.PI;          // flip so the star faces down
    ins.position.set(sx, -0.47, -1.8);
    g.add(ins);
  }
