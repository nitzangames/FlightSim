...
export function buildP51(THREE) {
  ...
  // a trapezoid with TOP edge (length 1.8) longer than BOTTOM edge (length
  // 1.0): the wide upper face fairs into the fuselage underside; the
  // narrower lower face is the radiator outlet. Front and rear faces
  // angle outward as they go up (fairing widens into the body).
  //
  ...
  scoopShape.moveTo(-0.5, 0.0);   // bottom-forward corner (short bottom edge)
  scoopShape.lineTo( 0.5, 0.0);   // bottom edge (length 1.0, radiator outlet)
  scoopShape.lineTo( 0.9, 0.5);   // angled rear fairing → top-rear
  scoopShape.lineTo(-0.9, 0.5);   // top edge (length 1.8, faired into fuselage)
  // ExtrudeGeometry implicitly closes back to start via the angled forward fairing.
