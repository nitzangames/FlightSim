...
export function buildF4(THREE) {
  ...
  bodyShape.lineTo( 5.75,  0.00);   // pointed tail tip
  bodyShape.lineTo( 4.75, -0.20);   // bottom trailing edge
  ...
  const body = new THREE.Mesh(bodyGeo, mat(GREEN));
  ...
  // Twin engine nacelles — side pods flanking the fuselage. They run from
  // the intake mouth (front) to just before the stabilizers, NOT all the
  // way to the back of the plane (matching the reference top view where
  // the engine bulges stop short of the tail surfaces). Exhaust nozzles
  // exit from the rear of the body proper, between the stabilizers.
  const intakeGeo = new THREE.BoxGeometry(0.70, 0.85, 4.90);
  for (const sx of [-0.95, 0.95]) {
    const intake = new THREE.Mesh(intakeGeo, mat(BROWN));
    intake.position.set(sx, -0.20, 0.05);
    g.add(intake);
  }
  ...
  for (const side of [-1, 1]) {
    ...
    // wingtip), y=-1.5 (mid-chord). ins.rotation.x = ±π/2 re-orients the
    // helper's Y-stacking direction along the wing's local Z so the disc
    // ends up flat against the wing surface after the wing's own rotations.
    const insTop = makeStarOfDavid(THREE, 0.7);
    insTop.rotation.x = Math.PI / 2;
    insTop.position.set(2.0, -1.5, 0.185);
    outer.add(insTop);
    const insBot = makeStarOfDavid(THREE, 0.7);
    insBot.rotation.x = -Math.PI / 2;
    insBot.position.set(2.0, -1.5, -0.005);
    outer.add(insBot);
  }
  ...
  // (X=±0.95 to align with the nacelles, Z=2.65 so the nozzle ring sits
  // flush with the nacelle's back face at Z=2.50).
  for (const sx of [-0.95, 0.95]) {
    const ex = new THREE.Mesh(new THREE.CylinderGeometry(0.32, 0.28, 0.55, 10), mat(STEEL));
    ex.rotation.x = Math.PI / 2;
    ex.position.set(sx, -0.20, 2.65);
    g.add(ex);
    const hole = new THREE.Mesh(new THREE.CircleGeometry(0.24, 12), mat(EXH));
    hole.rotation.y = Math.PI;
    hole.position.set(sx, -0.20, 2.93);
    g.add(hole);
  }
