...
export function buildF4(THREE) {
  ...
  // the wing-root height. F-4 signature feature: they DOMINATE the front
  // view in the reference, so sized prominently and shifted outboard.
  const intakeGeo = new THREE.BoxGeometry(0.85, 0.95, 2.8);
  ...
  for (const sx of [-1.10, 1.10]) {
    const face = new THREE.Mesh(new THREE.PlaneGeometry(0.70, 0.80), mat(DARK));
