...
export function buildF4(THREE) {
  ...
  // Twin exhaust nozzles — emerge from the back of the body between the
  // stabilizers (NOT from the nacelles, which stop further forward).
  ...
  // Star of David insignia — wing tops + bottoms only (no fuselage sides
  // in the reference). Wing inner panel world Y range: [-0.27, -0.09].
  for (const sx of [-1.6, 1.6]) {
    const ins = makeStarOfDavid(THREE, 0.6);
