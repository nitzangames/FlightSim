...
export function buildF4(THREE) {
  ...
  // Engine-bay cylinder: radius 0.70, spans world Z from -2.00 to +1.50
  // (shorter than before so the back cone can start more forward and
  // extend further aft for a longer tail).
  const bodyCylinder = new THREE.Mesh(
    new THREE.CylinderGeometry(0.70, 0.70, 3.50, 14, 1, true), mat(GREEN),
  );
  bodyCylinder.rotation.x = Math.PI / 2;
  bodyCylinder.position.z = -0.25;
  g.add(bodyCylinder);

  // Back cone: longer pointy tail. Base radius 0.70 matches the cylinder
  // at z=1.50; tip at world z=6.50.
  const backCone = new THREE.Mesh(
    new THREE.ConeGeometry(0.70, 5.00, 14, 1, true), mat(GREEN),
  );
  backCone.rotation.x = Math.PI / 2;
  backCone.position.z = 4.00;
  g.add(backCone);
  ...
  for (const side of [-1, 1]) {
    ...
    s.position.set(side * 1.4, 0.05, 3.6);
