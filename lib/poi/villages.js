...
const TEMPLATES = [FOREST_TEMPLATE, DESERT_TEMPLATE, ARCTIC_TEMPLATE, CASTLE_TEMPLATE];
...
// A template may declare `biome` (single string) OR `biomes` (array) — castles
// use the array form to live in multiple biomes.
function matchTemplate(biomeKey, altitude) {
  for (const t of TEMPLATES) {
    const biomeOK = t.biomes
      ? t.biomes.includes(biomeKey)
      : (t.biome === biomeKey);
    if (!biomeOK) continue;
