...
// Road segment — a flat 1×0.05×1 unit slab. Instances are scaled to the road
// width and length (the InstancedMesh sets scaleX = width, scaleZ = length).
// Lies just above terrain. Whole geometry tagged role=0 (uses wallColor) so
// the per-instance "wall" tint paints the road color.
export function buildRoadGeometry(THREE) {
  const v = [], n = [], r = [];
