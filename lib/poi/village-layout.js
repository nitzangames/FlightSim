...
function layoutCastle(village) {
  ...
  // Mesh-rotation convention: the position formula below uses 2D math
  // CCW rotation, but Three.js's mesh.rotation.y is the OPPOSITE sense
  // (CW from above) — so to make a mesh face the same way the position
  // formula says it should, we apply NEGATIVE baseAngle to the mesh.
  // Towers and keep are square (rotation-symmetric) so this sign change
  // is invisible on them; the wall mesh below is asymmetric and is the
  // one that actually depends on getting this right.
  const meshAngle = -baseAngle;

  // Keep at centre.
  add('castle_keep', village.x, village.z, meshAngle, sScale());
  ...
  for (const [lx, lz] of corners) {
    ...
    add('castle_tower', x, z, meshAngle, sScale());
  }

  // 4 wall segments midway between each pair of corners. The wall mesh
  // is built by `pushWallsNoTop(v, n, r, 0.8, 3.5, 6, ...)` whose third
  // arg (`d=6`) is the Z extent — so the wall's LONG axis is local Z,
  // and the scaleZ stretch (wallScaleZ ≈ 7.33) extends that axis to
  // bridge between corner towers (2 × HALF_CORNER = 44 m).
  //
  // To run a wall along world X (south / north sides — where the two
  // corners differ in X), local Z must map to world X → rotY = π/2.
  // To run it along world Z (east / west sides — corners differ in Z),
  // local Z stays at world Z → rotY = 0.
  ...
  for (const [lx, lz, dRot] of sides) {
    const x = village.x + lx * cosA - lz * sinA;
    const z = village.z + lx * sinA + lz * cosA;
    // Use meshAngle (= -baseAngle) so the wall's long axis aligns with the
    // line between the two corner towers it bridges — see the rotation-
    // convention note where meshAngle is defined.
    add('castle_wall', x, z, meshAngle + dRot, sScale(), wallScaleZ);
  }
...
function layoutMonastery(village) {
  ...
  function add(type, lx, lz, dRotY, scale) {
    ...
    const z = village.z + lx * sinA + lz * cosA;
    out.push({
      // Mesh rotation needs the NEGATIVE of the position formula's angle —
      // same convention as layoutCastle's meshAngle (see the note there).
      // With +baseAngle the wings rotate opposite to the ring they sit on,
      // scrambling the cloister U and pushing wing tips off the flat pad.
