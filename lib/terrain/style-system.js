...
export const STYLES = {
  ...
  // Topographic: cartograph's cousin — biome color base, plus contour lines
  // every 30m and major contours every 150m. Sky stays light/cream so the
  // contours read cleanly like a paper map.
  topographic: {
    sky:          [0.85, 0.86, 0.84],
  ...
  // Pencil sketch: desaturate biome color toward paper white, then add
  // cross-hatch ink lines that get denser in darker areas. World-space hatching
  // so the lines stay fixed against the moving plane.
  pencil: {
    sky:          [0.95, 0.93, 0.86],
    skyTop:       [0.92, 0.90, 0.83],
    skyHorizon:   [0.97, 0.95, 0.88],
    skyExponent:  0.50,
    fogType:      'linear',
    fogColor:     [0.96, 0.94, 0.87],
    fogNear:      1200,
    fogFar:       3500,
    waterColor:   [0.55, 0.58, 0.62],
    waterOpacity: 1.0,
    sunColor:     [1.00, 0.99, 0.95],
    hemiSky:      [0.95, 0.93, 0.86],
    hemiGround:   [0.78, 0.76, 0.70],
    hemiIntensity: 0.50,
  },
};
...
const STYLE_INDEX = { lowpoly: 0, stylized: 1, realistic: 2, cartograph: 3, topographic: 4, pencil: 5 };
    ...
    shader.fragmentShader = shader.fragmentShader.replace(
      ...
      `#include <color_fragment>
       if (uStyle == 0) {
         ...
         // Topographic: biome color base + contour lines.
         //   Minor contours every 30m (thin)
         //   Major contours every 150m (thicker, darker)
         float y = vWorldPosT.y;
         float minorOff = abs(mod(y + 15.0, 30.0) - 15.0);
         float majorOff = abs(mod(y + 75.0, 150.0) - 75.0);
         // Width of each line in meters
         float minorLine = 1.0 - smoothstep(0.4, 1.6, minorOff);
         float majorLine = 1.0 - smoothstep(0.8, 3.5, majorOff);
         float lineAmt = clamp(minorLine * 0.35 + majorLine * 0.75, 0.0, 0.85);
         vec3 lineColor = vec3(0.12, 0.16, 0.20);
         diffuseColor.rgb = mix(diffuseColor.rgb, lineColor, lineAmt);
       } else if (uStyle == 5) {
         // Pencil sketch: grayscale base toward paper white + cross-hatch
         // ink lines that get denser in darker (lower-elevation/shadow) areas.
         float gray = dot(diffuseColor.rgb, vec3(0.30, 0.59, 0.11));
         vec3 paper = vec3(0.94, 0.92, 0.86);
         vec3 ink   = vec3(0.14, 0.14, 0.18);
         float darkness = clamp(1.0 - gray, 0.0, 1.0);
         // Two world-space hatch directions (45° apart), ~3m period.
         float hA = abs(fract((vWorldPosT.x + vWorldPosT.z) * 0.33) - 0.5);
         float hB = abs(fract((vWorldPosT.x - vWorldPosT.z) * 0.33) - 0.5);
         // Thin lines: ink at the boundary (distance near 0 from line center)
         float lineA = 1.0 - smoothstep(0.04, 0.10, hA);
         float lineB = 1.0 - smoothstep(0.04, 0.10, hB);
         // First hatch direction appears past darkness 0.25; cross-hatch past 0.55
         float ha = lineA * smoothstep(0.25, 0.45, darkness);
         float hb = lineB * smoothstep(0.55, 0.75, darkness);
         float inkAmt = clamp(darkness * 0.30 + ha * 0.55 + hb * 0.55, 0.0, 1.0);
         diffuseColor.rgb = mix(paper, ink, inkAmt);
       }`
    );
