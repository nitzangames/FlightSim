...
export function createTerrain(opts) {
  ...
  // Buildings are 2.5x scale and a village is 200–300 m wide, so the player
  // needs to see them from much farther than trees. Push the shrink-fade out
  // to 900–1200 m — well inside the LOD-0 chunk emission range (~1024 m at
  // the worst-case chunk corner) so buildings stay full size until the chunk
  // unloads at the LOD-1 boundary.
  buildingMaterial.userData.uFadeStart = { value: 900 };
  buildingMaterial.userData.uFadeEnd   = { value: 1200 };
  ...
  windmillBladeMaterial.userData.uFadeStart = { value: 900 };
  windmillBladeMaterial.userData.uFadeEnd   = { value: 1200 };
  ...
  return {
    ...
    villageRegistry,
    nearestVillage(x, z) {
      // Linear scan over ~50 villages — cheap. Returns { x, z, distance } or null.
      const all = villageRegistry.all;
      ...
      return { x: best.x, z: best.z, distance: Math.sqrt(bestD2) };
    },
    getRiverWidthAt(x, z) {
