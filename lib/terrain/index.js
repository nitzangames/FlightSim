...
export function createTerrain(opts) {
  ...
  // Fly-through markers — one ring per landmark (currently villages, will
  // extend to castles/monasteries later). Generic input: { id, x, y, z, angle }.
  const landmarks = villageRegistry.all
    .filter(v => v.runway)
    .map(v => ({
      id: `v:${v.id}`,
      x: v.runway.x, y: v.runway.y, z: v.runway.z,
      angle: v.runway.angle,
    }));
