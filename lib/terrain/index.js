...
export function createTerrain(opts) {
  ...
  // Fly-through markers — one ring per landmark. Villages get a runway-
  // aligned vertical ring; castles get a horizontal ring (no canonical
  // approach direction).
  const landmarks = [];
  for (const v of villageRegistry.all) {
    if (v.runway) {
      landmarks.push({
        id: `v:${v.id}`,
        x: v.runway.x, y: v.runway.y, z: v.runway.z,
        angle: v.runway.angle, horizontal: false,
      });
    } else if (v.markerTarget) {
      landmarks.push({
        id: `v:${v.id}`,
        x: v.markerTarget.x, y: v.markerTarget.y, z: v.markerTarget.z,
        angle: 0, horizontal: !!v.markerTarget.horizontal,
      });
    }
  }
