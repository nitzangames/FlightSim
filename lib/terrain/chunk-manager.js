...
import { buildTreeInstancedMesh, buildBillboardInstancedMesh, BILLBOARD_BIOME_STYLE } from './trees.js';
...
export class ChunkManager {
  ...
  _install(key, cx, cz, lod, out) {
    ...
    if (out.trees && out.trees.length > 0 && lod === 0) {
      ...
      if (!skipScatter) {
        ...
        if (scatterList.length > 0) {
          ...
          // LOD-0 overlay billboards: same positions as the close-up trees,
          // but with INVERTED fade (visible at the far edge of LOD 0, invisible
          // close-in). As the camera approaches, the tree grows from 0 and the
          // billboard fades to 0 over the same 600–750 m band, keeping the
          // silhouette continuous through the LOD 1→0 transition.
          if (this.billboardMaterialNear && this.billboardGeometry) {
            const biomeAtCenter = this.biomeAt
              ? this.biomeAt((cx + 0.5) * CHUNK_SIZE, (cz + 0.5) * CHUNK_SIZE)
              : null;
            const style = (biomeAtCenter && BILLBOARD_BIOME_STYLE[biomeAtCenter.name]) || BILLBOARD_BIOME_STYLE.forest;
            const overlay = scatterList.map(t => ({
              x: t.x, y: t.y, z: t.z, scale: t.scale,
              color: style.color, shape: style.shape,
            }));
            overlayBillboardMesh = buildBillboardInstancedMesh(THREE, this.billboardGeometry, this.billboardMaterialNear, overlay);
            this.group.add(overlayBillboardMesh);
          }
        }
      }
    }
    ...
    if (out.buildings && this.buildingMaterial && this.buildingGeometries) {
      const NORMAL_TYPES = ['house', 'barn', 'church', 'windmill'];
      ...
      for (const type of NORMAL_TYPES) {
        ...
        const baseGeom = this.buildingGeometries[type];
    ...
    this.resident.set(key, { mesh, treeMesh, overlayBillboardMesh, buildingMeshes, lod, cx, cz });
  }
  ...
  _dispose(entry) {
    ...
    if (entry.overlayBillboardMesh) {
      this.group.remove(entry.overlayBillboardMesh);
      entry.overlayBillboardMesh.geometry.dispose();
    }
