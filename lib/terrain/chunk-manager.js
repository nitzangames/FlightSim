...
import { densityAt, biomeDominanceAt } from '../game/biomes.js';

// Scatter (trees / cacti / etc.) is only emitted when the chunk's biome
// blend is dominated by a single biome above this fraction. In transition
// zones below this threshold the ground colours bilerp across multiple
// biomes — placing one biome's trees there would clash visually with the
// blended colours and read as "wrong biome trees".
const SCATTER_DOMINANCE_THRESHOLD = 0.45;
...
export class ChunkManager {
  constructor({ THREE, scene, runner, terrainMaterial, treeMaterial, treeGeometry,
                ...
                villageRegistry = null, buildingMaterial = null,
  ...
  _enqueue(key, lod) {
    ...
    this.runner.build({ cx, cz, lod, vertexGrid: grid, villages }).then((out) => {
  ...
  _install(key, cx, cz, lod, out) {
    ...
    if (out.trees && out.trees.length > 0 && (lod === 0 || lod === 1)) {
      ...
      if (this.biomeAt && this.scatterGeometries) {
        ...
        // Use the DOMINANT biome (winning weight across the 4-cell blend)
        // rather than the discrete chunk-centre lookup. Skip scatter when
        // no single biome owns at least SCATTER_DOMINANCE_THRESHOLD of the
        // chunk — that's the transition zone, and any biome's trees there
        // would clash with the bilerp-blended ground colours.
        const dom = biomeDominanceAt(centerX, centerZ);
        if (dom.weight < SCATTER_DOMINANCE_THRESHOLD) {
          skipScatter = true;
        }
        const biome = dom.biome;
        if (!skipScatter && biome.scatterKey === null) skipScatter = true;
        if (!skipScatter) {
  ...
  _dispose(entry) {
    ...
    if (entry.treeMesh) {
      ...
      // The geometry is the registry-owned shared tree/scatter geometry —
      // every other resident tree chunk still references it, so it must NOT
      // be disposed here. The InstancedMesh itself owns the per-chunk
      // instanceMatrix GL buffer, which only an explicit dispose() releases.
      entry.treeMesh.dispose();
    }
    if (entry.buildingMeshes) {
      for (const im of entry.buildingMeshes) {
        ...
        // dispose() on the mesh additionally frees the instanceMatrix buffer.
        im.geometry.dispose();
        im.dispose();
      }
    }
