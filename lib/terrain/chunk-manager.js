    ...
    if (out.trees && out.trees.length > 0 && lod === 0) {
      ...
      if (this.biomeAt && this.scatterGeometries) {
        ...
        if (biome.scatterKey === null) skipScatter = true;
          ...
          // Per-biome density tuning. Cacti are sparse in real deserts; trees
          // are halved so forests/autumn don't look like impenetrable canopy.
          if (biome.scatterKey === 'cactus') densityFactor = 0.10;
          else if (biome.scatterKey === 'conifer' || biome.scatterKey === 'maple') densityFactor = 0.50;
        }
      }
      if (!skipScatter) {
