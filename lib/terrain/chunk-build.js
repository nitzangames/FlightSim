...
// `biomeAt` is optional — if provided, each vertex's color comes from the
// biome's full per-altitude band palette at that world position.
export function buildChunkBuffers({ cx, cz, lod, seed, riverSegments, vertexGrid, biomeAt }) {
  ...
  for (let j = 0; j < N; j++) {
    for (let i = 0; i < N; i++) {
      ...
      const bands = biomeAt ? biomeAt(cx0, cz0).bands : DEFAULT_BANDS;
      const col = colorForHeight(cy0, bands);
      colors[p]     = col[0];
      colors[p + 1] = col[1];
      colors[p + 2] = col[2];
    }
