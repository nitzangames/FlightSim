...
// Writes the per-altitude band color into `colors[p..p+2]`. Above water,
// blends smoothly through grassLow → grassMid → rock → snow. `yJittered` is
// the height plus a smooth per-vertex perturbation so the boundaries aren't
// perfectly horizontal across the world.
function writeColorForHeight(colors, p, yJittered, bands) {
  if (yJittered < WATER_LEVEL - 1) {
    colors[p]     = bands.deepWater[0];
    colors[p + 1] = bands.deepWater[1];
    colors[p + 2] = bands.deepWater[2];
    return;
  }
  if (yJittered < WATER_LEVEL + 0.5) {
...
export function buildChunkBuffers({ cx, cz, lod, seed, riverSegments, vertexGrid, biomeAt }) {
  ...
  // Pick the biome ONCE for this whole chunk (from its center) so neighbouring
  // biomes don't leak their bands into our vertices.
  const chunkBands = biomeAt
    ? biomeAt(x0 + CHUNK_SIZE * 0.5, z0 + CHUNK_SIZE * 0.5).bands
    : DEFAULT_BANDS;
  ...
  for (let j = 0; j < N; j++) {
    for (let i = 0; i < N; i++) {
      ...
      writeColorForHeight(colors, p, cy0 + yJitter, chunkBands);
    }
  }
