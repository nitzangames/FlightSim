...
function colorForHeight(y, bands) {
  if (y < WATER_LEVEL - 1)   return bands.deepWater;
  if (y < WATER_LEVEL + 0.5) return bands.sand;
  if (y > SNOW_LINE)         return bands.snow;
  if (y > ROCK_LINE)         return bands.rock;
  if (y > GRASS_MID_LINE)    return bands.grassMid;
  return bands.grassLow;
}
...
export function buildChunkBuffers({ cx, cz, lod, seed, riverSegments, vertexGrid, biomeAt }) {
  ...
  for (let j = 0; j < N; j++) {
    for (let i = 0; i < N; i++) {
      ...
      const col = colorForHeight(cy0, chunkBands);
      colors[p]     = col[0];
      colors[p + 1] = col[1];
      colors[p + 2] = col[2];
    }
