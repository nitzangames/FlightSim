...
// Angular warp of a village's pad radius so the flattened footprint reads as an
// irregular blob instead of a perfect circle. Returns a multiplier in
// [1, 1+PAD_WARP_AMP]: OUTWARD-ONLY because buildings are scattered out to
// ~0.92·padRadius in every direction (see village-layout), so the flat region
// can't shrink below that anywhere without stranding a building on a slope. The
// inner disc therefore stays flat; only the outer edge bulges into lobes. Smooth
// 2nd+3rd harmonics keep it continuous at the 0/2π wrap; phases come from the
// village's paletteSeed so each one is a different shape, deterministically.
const PAD_WARP_AMP = 0.45;
function padWarp(V, dx, dz) {
  const a = Math.atan2(dz, dx);
  const p1 = V.paletteSeed * 6.2831853;
  const p2 = V.paletteSeed * 19.739 + 1.7;
...
export function buildChunkBuffers({ cx, cz, lod, seed, riverSegments, vertexGrid, biomeAt, bandsAt, villages = [] }) {
  ...
  if (villages.length > 0) {
    for (let i = 0; i < totalInterior; i++) {
      ...
      for (const V of villages) {
        ...
        // Warp the pad/falloff radii by angle so the flattened edge isn't a
        // perfect circle. Cheap early-out on the un-warped max reach first.
        if (d >= V.falloffRadius * (1 + PAD_WARP_AMP)) continue;
        const w = padWarp(V, dx, dz);
        const padR = V.padRadius * w, falloffR = V.falloffRadius * w;
        if (d >= falloffR) continue;
        const u = (falloffR - d) / (falloffR - padR);
  ...
  const trees = (lod === 0 || lod === 1) ? placeTrees(cx, cz, seed, localSegs, TREE_GRID_PITCH, villages) : [];
  const billboards = [];
  ...
  for (let j = 0; j < CHUNK_SIZE; j += pitch) {
    for (let i = 0; i < CHUNK_SIZE; i += pitch) {
      ...
      for (const V of villages) {
        ...
        // Match the warped flatten footprint so trees don't sit on the bulges.
        const padR = V.padRadius * padWarp(V, dx, dz) + 4;
