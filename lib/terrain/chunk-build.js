...
import { flattenGroundHeight, padWarp } from './pad-flatten.js';
...
export function buildChunkBuffers({ cx, cz, lod, seed, riverSegments, vertexGrid, biomeAt, bandsAt, villages = [] }) {
  ...
  // Lerp each vertex toward the nearby villages' groundY using the SHARED
  // pad-flatten function (see pad-flatten.js) — the exact same function the
  // collision/HUD path calls in terrain index getHeight, so the rendered
  // ground and the collision surface are identical. Runs BEFORE river carve so
  // carving still wins where it overlaps (placement excluded villages on
  // rivers, so no conflict in practice — this is defense-in-depth).
  if (villages.length > 0) {
    for (let i = 0; i < totalInterior; i++) {
      const vx = positions[i * 3];
      const vz = positions[i * 3 + 2];
      positions[i * 3 + 1] = flattenGroundHeight(positions[i * 3 + 1], vx, vz, villages);
    }
  }
...
function placeTrees(cx, cz, seed, localSegs, pitch, villages = []) {
  ...
  for (let j = 0; j < CHUNK_SIZE; j += pitch) {
    for (let i = 0; i < CHUNK_SIZE; i += pitch) {
      ...
      // Pass through the village pad flatten so trees in the pad-to-falloff
      // ring stand on the blended ground, not the raw terrain — on hilly
      // terrain the delta is metres, leaving trees hovering or swallowed.
      const y = flattenGroundHeight(terrainHeight(x, z, seed), x, z, villages);
      ...
      for (const V of villages) {
        ...
        const padR = V.padRadius * padWarp(V.paletteSeed, dx, dz) + 4;
