...
import { BILLBOARD_BIOME_STYLE } from './trees.js';
...
export function buildChunkBuffers({ cx, cz, lod, seed, riverSegments, vertexGrid, biomeAt, bandsAt, villages = [] }) {
  ...
  // Biome-tint LOD-1 billboards. One color/shape per chunk (sampled at center)
  // — at this resolution mixed-biome chunks are rare and tolerable, and avoids
  // a per-vertex biomeAt call. Color/shape are picked from the biome name; an
  // unknown biome falls back to forest conifer (green pointy triangle).
  if (lod === 1 && billboards.length > 0) {
    const chunkBiome = biomeAt
      ? biomeAt(x0 + CHUNK_SIZE * 0.5, z0 + CHUNK_SIZE * 0.5)
      : null;
    const style = (chunkBiome && BILLBOARD_BIOME_STYLE[chunkBiome.name]) || BILLBOARD_BIOME_STYLE.forest;
    for (const b of billboards) { b.color = style.color; b.shape = style.shape; }
  }
  ...
  if (lod === 0 && villages.length > 0) {
    ...
    for (const V of villages) {
      ...
      for (const b of instances) {
        ...
        // Each windmill spawns 4 blade instances at 90° offsets around the
        // local Z axis. The hub sits 5.5 m above the windmill base.
        if (b.type === 'windmill') {
          for (let i = 0; i < 4; i++) {
