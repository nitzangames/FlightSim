...
// The base shape uses three octaves (hills + mountains + basins). The biome
// system then scales the result smoothly via heightScaleAt — desert is flat
// (scale ~0.35), arctic is jagged (~1.5), etc.
export function terrainHeight(x, z, worldSeed = 0) {
  ...
  return (base + mountains + basins - 12) * heightScaleAt(x, z);
}
